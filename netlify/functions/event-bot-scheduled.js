// Scheduled: runs every 6 hours to generate upcoming events
// Netlify config: cron = "0 */6 * * *"
...
Retourne UNIQUEMENT un JSON strict sous ce format (pas de markdown, pas de texte avant/après) :
{
  ...
  try {
    ...
    console.error('[event-bot-scheduled] parse error:', e, 'raw:', raw.slice(0, 500));
    return [];
  }
}
...
async function insertEvents(supabase, events) {
  ...
  for (const ev of events) {
    ...
    if (error) {
      console.error('[event-bot-scheduled] insert error:', error);
      failed++;
      continue;
    }
...
exports.handler = async function(event, context) {
  ...
  const categories = ['Religion', 'Conférence', 'Impact', 'Entrepreneuriat', 'Business', 'Economie'];
  const countPerCat = 2;
  ...
  return { statusCode: 200, body: JSON.stringify({ ok: true, created, failed, total: allEvents.length }) };
};
