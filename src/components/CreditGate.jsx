...
export default function CreditGate({ type, produitId, children, onAccess }) {
  ...
  return (
    <div style={{ padding: '24px', borderRadius: '16px', background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.2)', textAlign: 'center' }}>
      ...
      <h3 style={{ color: '#F0F2F5', fontWeight: 800, marginBottom: '6px' }}>Crédits insuffisants</h3>
      <p style={{ color: '#8B95A5', fontSize: '0.85rem', marginBottom: '16px' }}>
        Ce contenu coûte <strong style={{ color: '#F0B429' }}>{cost} crédits</strong><br/>
        Votre solde : <strong style={{ color: '#ef4444' }}>{solde} crédits</strong>
      </p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        <Link to="/credits" style={{ padding: '14px', borderRadius: '12px', textDecoration: 'none', background: 'linear-gradient(135deg, #F0B429, #e5a820)', color: '#070B0F', fontWeight: 800 }}>💳 Recharger mes crédits</Link>
        <Link to="/plans" style={{ padding: '12px', borderRadius: '10px', textDecoration: 'none', background: 'rgba(139,92,246,0.08)', border: '1px solid rgba(139,92,246,0.2)', color: '#8B5CF6', fontWeight: 700 }}>💎 ABAWI+ — 200 crédits/mois inclus</Link>
      </div>
    </div>
  )
}
