...
const field = { width: '100%', borderRadius: 10, border: '1px solid var(--color-border)', background: 'var(--color-bg-primary)', color: 'var(--color-text-primary)', padding: '10px 12px', fontSize: '0.85rem', fontFamily: 'Outfit,sans-serif' }
const label = { display: 'block', color: 'var(--color-text-secondary)', fontSize: '0.75rem', marginBottom: 6, fontWeight: 700, letterSpacing: 0.5 }
...
STYLE VISUEL (palette couleurs, typographie, iconographie).`
  ...
  return (
    <div style={{ display: 'grid', gap: 18 }}>
      <section style={{ border: '1px solid var(--color-border)', background: 'linear-gradient(135deg, rgba(236,72,153,0.14), rgba(2,6,23,0.75))', borderRadius: 16, padding: 18 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 14, alignItems: 'center', flexWrap: 'wrap' }}>
          <div>
            <h3 style={{ margin: '0 0 6px', color: 'var(--color-text-primary)' }}>Autonomie Marketing Expert</h3>
            <p style={{ margin: 0, color: 'var(--color-text-secondary)', fontSize: '0.85rem' }}>Veille IA, recommandations, déclenchement campagnes validées et studio infographie publicitaire.</p>
          </div>
      ...
      <section style={{ border: '1px solid var(--color-border)', borderRadius: 16, background: 'var(--color-bg-secondary)', padding: 16 }}>
        ...
        <div style={{ marginTop: 12, display: 'flex', gap: 10 }}>
          <button onClick={runStrategicWatch} disabled={loading || !canRun} style={{ borderRadius: 10, border: 'none', padding: '10px 14px', fontWeight: 800, cursor: loading ? 'not-allowed' : 'pointer', background: loading ? 'var(--color-border)' : 'linear-gradient(135deg,#EC4899,#be185d)', color: '#fff' }}>
      ...
      {analysis && (
        <>
          <section style={{ border: '1px solid var(--color-border)', borderRadius: 16, background: 'var(--color-bg-secondary)', padding: 16 }}>
            <h4 style={{ marginTop: 0, color: 'var(--color-text-primary)' }}>Signaux marché détectés</h4>
            {(analysis.marketSignals || []).map((s, i) => (
              <div key={i} style={{ color: 'var(--color-text-primary)', fontSize: '0.84rem', marginBottom: 6 }}>• {s}</div>
            ))}
          </section>

          <section style={{ border: '1px solid var(--color-border)', borderRadius: 16, background: 'var(--color-bg-secondary)', padding: 16 }}>
            <h4 style={{ marginTop: 0, color: 'var(--color-text-primary)' }}>Campagnes proposées par l’IA</h4>
            <div style={{ display: 'grid', gap: 12 }}>
              {(analysis.campaigns || []).map((camp, i) => (
                <div key={`${camp.name}-${i}`} style={{ border: '1px solid var(--color-border-hover)', borderRadius: 12, padding: 12, background: 'var(--color-bg-card)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}>
                    <div>
                      <div style={{ color: 'var(--color-text-primary)', fontWeight: 800 }}>{camp.name}</div>
                      <div style={{ color: 'var(--color-text-secondary)', fontSize: '0.8rem' }}>{camp.platform} · budget {Number(camp.budget || 0).toLocaleString('fr')} FCFA · {camp.timelineDays || 14} jours</div>
                      <div style={{ color: 'var(--color-text-primary)', fontSize: '0.82rem', marginTop: 4 }}>{camp.goal}</div>
                    </div>
                  ...
                  {!!camp.kpi?.length && <div style={{ marginTop: 8, color: 'var(--color-text-secondary)', fontSize: '0.78rem' }}>KPI: {camp.kpi.join(' • ')}</div>}
                </div>
              ))}
            </div>
          </section>
          ...
          <section style={{ border: '1px solid var(--color-border)', borderRadius: 16, background: 'var(--color-bg-secondary)', padding: 16 }}>
            <h4 style={{ marginTop: 0, color: 'var(--color-text-primary)' }}>Conseils IA actionnables</h4>
            {(analysis.advice || []).map((a, i) => (
              <div key={i} style={{ color: 'var(--color-text-primary)', fontSize: '0.84rem', marginBottom: 6 }}>• {a}</div>
            ))}
          </section>
        </>
      )}
      ...
      {(selectedCampaign || creative) && (
        <section style={{ border: '1px solid rgba(240,180,41,0.2)', borderRadius: 16, background: 'rgba(240,180,41,0.04)', padding: 16 }}>
          ...
          {selectedCampaign && <p style={{ color: 'var(--color-text-secondary)', fontSize: '0.82rem' }}>Campagne: {selectedCampaign.name}</p>}
      ...
      {campaignPreview && (
        <section style={{ border: '1px solid rgba(59,130,246,0.3)', borderRadius: 16, background: 'rgba(59,130,246,0.06)', padding: 16 }}>
          ...
          <div style={{ color: 'var(--color-text-primary)', fontSize: '0.82rem', marginBottom: 8 }}>
            Score: {campaignPreview.simulation?.readinessScore ?? '—'} · Recommandation: {campaignPreview.simulation?.goLiveRecommendation || '—'}
          </div>
          <div style={{ background: 'var(--color-bg-primary)', border: '1px solid var(--color-border)', borderRadius: 10, padding: 12, color: 'var(--color-text-primary)' }}>
            <div><strong style={{ color: 'var(--color-text-primary)' }}>Hook:</strong> {campaignPreview.simulation?.finalRender?.hook || '—'}</div>
            <div style={{ marginTop: 6 }}><strong style={{ color: 'var(--color-text-primary)' }}>Body:</strong> {campaignPreview.simulation?.finalRender?.body || '—'}</div>
            <div style={{ marginTop: 6 }}><strong style={{ color: 'var(--color-text-primary)' }}>CTA:</strong> {campaignPreview.simulation?.finalRender?.cta || '—'}</div>
          </div>
          {!!campaignPreview.simulation?.adjustments?.length && (
            <div style={{ marginTop: 8, color: 'var(--color-text-secondary)', fontSize: '0.78rem' }}>
